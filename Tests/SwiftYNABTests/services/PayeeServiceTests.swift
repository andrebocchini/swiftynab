...
struct PayeeServiceTests {
    ...
    func payeesThrowsErrorWhenRequestFails() async {
    ...
    @Test("Returns a single payee when request succeeds")
    func payeeReturnsPayeeWhenRequestSucceeds() async throws {
        let expectedPayee = Payee(id: "id", name: "name", transferAccountId: nil, deleted: false)
    ...
    @Test("Throws error when fetching a single payee fails")
    func payeeThrowsErrorWhenRequestFails() async {
        let expectedError = SwiftYNABError.httpError(statusCode: 500)
        let client = MockFailureClient(expectedError: expectedError)
        let service = PayeeService(client: client)
    ...
    @Test("Returns updated payee when update request succeeds")
    func updatePayeeReturnsPayeeWhenRequestSucceeds() async throws {
        let expectedPayee = Payee(id: "id", name: "name", transferAccountId: nil, deleted: false)
    ...
    @Test("Throws error when updating a payee fails")
    func updatePayeeThrowsErrorWhenRequestFails() async {
        let expectedError = SwiftYNABError.httpError(statusCode: 500)
        let client = MockFailureClient(expectedError: expectedError)
        let service = PayeeService(client: client)
